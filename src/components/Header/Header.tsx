...
const Header = () => {
  ...
  return (
    <header className="z-50 bg-gray-950 px-6 py-3 flex justify-between items-center sticky w-full top-0 shadow-lg">
      ...
      <div className="flex items-center gap-4 pl-2">
        {isLoggedIn ? (
          <div className="flex items-center gap-4">
            <Link 
              to="/cart" 
              className="relative text-white hover:text-purple-400 transition-colors flex items-center justify-center group"
            >
              {cartCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full group-hover:animate-bounce">
                  {cartCount}
                </span>
              )}
      ...
      {isMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-gray-950 border-t border-gray-800 shadow-xl animate-fadeIn">
          {/* Mobile Search */}
          <div className="p-4">
            ...
            {['certificate', 'courses', 'majors', 'job', 'about'].map((tab) => (
              <Link
                ...
                className={`px-6 py-4 transition-all duration-200 ${
                  activeTab === tab 
                    ? 'bg-gray-800 text-purple-400 border-l-4 border-purple-400' 
                    : 'text-white hover:bg-gray-800 hover:text-purple-400 hover:border-l-4 hover:border-purple-400/50'
                }`}
