...
const UploadExamTemplate = ({ refetchExams }: { refetchExams: () => void }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "https://certificateinformationportal.azurewebsites.net/api/v1/template-quiz-exam/upload-exam-template",
        formData,
        {
          headers: {
